...
flowchart TD
    ...
    E["订单支付前 RevenueCalculator"] --> F["按 pay_channel 判断是否启用分账"]
...
| `/management/revenue.revenue_rule/getList` | 查询配置列表，返回兼容字段 `rr_id/rule_name/machine_num/receivers`。 | 查 `revenue_rule_config`，统计 `revenue_rule_config_scope` |
| `/management/revenue.revenue_rule/getFind` | 查询单个配置详情，包含 `receivers`。 | 查 `revenue_rule_config` |
