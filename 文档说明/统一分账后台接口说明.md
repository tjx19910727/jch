...
flowchart TD
    ...
    E["订单支付前 RevenueCalculator"] --> F["按最终 pay_channel 判断是否启用分账"]
...
| `/management/revenue.revenue_pay_channel/add` | 新增分账触发渠道。可传 `pay_channel`，也可只传 `pay_type`，系统按订单分类规则自动生成 `pay_channel/channel_name`。 | 写 `revenue_pay_channel` |
| `/management/revenue.revenue_pay_channel/update` | 修改分账触发渠道。传 `pay_type` 且未传有效 `pay_channel` 时，自动推导 `pay_channel/channel_name`。 | 写 `revenue_pay_channel` |
