...
{
    public function getStrategyPayeeValue($where,$value)
    {
        return StrategyPayeeModel::getFieldValue($where,$value);
    }
     ...
     * @return array
     */
    public function getStrategyPayeeContent($where,$field = "*",$order = "sm.sort asc, update_time desc")
    {
     ...
     * @return array
     */
    public function getStrategyPayeeContentByMachineId($m_id,$payee_type)
    {
        $s_ids = $this->getStrategyMachineColumn(['m_id' => $m_id,"s_type" => 1],'s_id');
        if (!$s_ids) return $this->rFail("当前设备未绑定收款方配置");
