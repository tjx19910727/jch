...
class Receive extends Common
{
    ...
    protected function initialize()
    {
        ...
        $postData = [];
        try {
            $requestData = json2arr(input());
            $postData = $this->normalizeReceivePostData($requestData);
            ...
            if ($message == "VReceive.timestamp_checkTimestamp_overdue") {
                $serverTime = time();
                $requestTimestamp = intval($postData['timestamp'] ?? 0);
                ...
                returnState(300, Lang::get($message), [
                    'server_time' => $serverTime,
                    'request_timestamp' => $requestTimestamp,
                    'server_time_offset' => $requestTimestamp ? $serverTime - $requestTimestamp : 0,
                    'timestamp_tolerance' => $timestampTolerance,
                ])->send();
                die();
            }
            returnState(300, Lang::get($message))->send();
            die();
        }
    }

    /**
     * 兼容设备将业务参数放在 data 节点中的 HTTP 包装格式。
     */
    protected function normalizeReceivePostData($requestData)
    {
        $requestData = is_array($requestData) ? $requestData : [];
        if (!array_key_exists('data', $requestData)) return $requestData;
        $data = json2arr($requestData['data']);
        if (!is_array($data)) return $requestData;
        if (!isset($data['msg_id']) && !isset($data['sign']) && !isset($data['timestamp'])) return $requestData;
        if (isset($requestData['machine_id']) && isset($data['machine_id'])
            && strval($requestData['machine_id']) !== strval($data['machine_id'])) {
            throw new \Exception('外层与data中的设备编号不一致');
        }
        if (!isset($data['machine_id']) && isset($requestData['machine_id'])) {
            $data['machine_id'] = $requestData['machine_id'];
        }
        return $data;
    }
