...
class SaleOrders extends Common
{
    ...
    public function getList()
    {
        ...
        $where = $this->getWhere($postData,false,['trade_no' => "like","order_type" => "in","mch_no" => "like","machine_name" => "like","machine_id" => "like","pay_type" => "in"]);
        $where['pay_status'] = 3;
    ...
    public function getDetailsList()
    {
        ...
        $field = "so.machine_id,so.machine_name,so.trade_no,so.mch_no,so.transaction_video,so.order_type,so.pay_type,so.pay_method,so.pay_time,so.out_time,so.create_time,so.out_status,so.refund_status,
