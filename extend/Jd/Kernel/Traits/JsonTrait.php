...
use const Jd\Kernel\Exceptions\ERR_INIT_CERTS_EXCLUDE_MCHSERIAL;
use const Jd\Kernel\Exceptions\ERR_INIT_CERTS_IS_MANDATORY;
use const Jd\Kernel\Exceptions\ERR_INIT_PRIVATEKEY_IS_MANDATORY;
use Jd\Kernel\Exceptions\InvalidArgumentException;
use const Jd\Kernel\Exceptions\ERR_INIT_MCHID_IS_MANDATORY;
use const Jd\Kernel\Exceptions\ERR_INIT_SERIAL_IS_MANDATORY;
use Jd\Kernel\Exceptions\WeChatPayException;
use Jd\Kernel\Support\Formatter;
use Jd\Kernel\Support\Rsa;
...
/**
 * JSON based TransferClient interface for sending HTTP requests.
 */
trait JsonTrait
{
    /**
    ...
    /**
     * Assert the HTTP `20X` responses fit for the business logic, otherwise thrown a `\GuzzleHttp\Exceptions\RequestException`.
     *
    ...
    public static function jsonBased(array $config = []): Client
    {
        ...
        )) { throw new InvalidArgumentException(ERR_INIT_MCHID_IS_MANDATORY); }

        if (!(
            isset($config['serial']) && is_string($config['serial'])
        )) { throw new InvalidArgumentException(ERR_INIT_SERIAL_IS_MANDATORY); }

        if (!(
            isset($config['privateKey']) && (is_string($config['privateKey']) || is_resource($config['privateKey']) || is_object($config['privateKey']))
        )) { throw new InvalidArgumentException(ERR_INIT_PRIVATEKEY_IS_MANDATORY); }

        if (!(
            isset($config['certs']) && is_array($config['certs']) && count($config['certs'])
        )) { throw new InvalidArgumentException(ERR_INIT_CERTS_IS_MANDATORY); }
