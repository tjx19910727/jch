...
namespace WeChatPayV3\Kernel\GuzzleMiddleware\Auth;

use Psr\Http\Message\ResponseInterface;
use WeChatPayV3\Kernel\GuzzleMiddleware\Validator;
...
class WechatPay2Validator implements Validator
{
    ...
    public function curl_validate($data)
    {
        $serialNo = $this->getCurlHeader($data,'Wechatpay-Serial');
        $sign = $this->getCurlHeader( $data,'Wechatpay-Signature');
        $timestamp = $this->getCurlHeader( $data,'Wechatpay-Timestamp');
        $nonce = $this->getCurlHeader( $data,'Wechatpay-Nonce');
        ...
        $body = $this->getCurlBody($data);
        $message = "$timestamp\n$nonce\n$body\n";
        return $this->verifier->verify($serialNo, $message, $sign);
    }

    protected function getCurlHeader($data,$name)
    {
        return $data['header'][$name] ?? '';
    }

    protected function getCurlBody($data)
    {
        return json_encode($data['body'],JSON_UNESCAPED_UNICODE);
    }
